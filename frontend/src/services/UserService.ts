...
const UserService = {
	...
	editDescription: async (evmAddress: string, description: string) => {
		const res = await fetch(`${ API_ENDPOINT }/api/user/edit/description`, {
			...
			body: JSON.stringify({ evmAddress: evmAddress, description: description }),
		});
		const data = await res.json();
		return data?.message;
	},
	...
	editEducations: async (evmAddress: string, educations: UserProps["educations"]) => {
		const res = await fetch(`${ API_ENDPOINT }/api/user/edit/educations`, {
			method: "post",
			headers: {
				"Content-Type": "application/json",
			},
			body: JSON.stringify({ evmAddress: evmAddress, educations: educations }),
		});
		const data = await res.json();
		return data?.message;
	},
	editCertifications: async (evmAddress: string, certifications: UserProps["certifications"]) => {
		const res = await fetch(`${ API_ENDPOINT }/api/user/edit/certifications`, {
			method: "post",
			headers: {
				"Content-Type": "application/json",
			},
			body: JSON.stringify({ evmAddress: evmAddress, certifications: certifications }),
		});
		const data = await res.json();
		return data?.message;
	},
