...
const GigService = {
	...
	perSubCategory: async (
		...
		const res = await fetch(`${ API_ENDPOINT }/gig/subcategory`, {
			...
			body: JSON.stringify({
				...
				sortField: sortField,
				sortOrder: sortOrder,
				selectableDeliverables: selectableDeliverables,
	...
	perHash: async (hash: string | null) => {
		const res = await fetch(`${ API_ENDPOINT }/gig/hash`, {
			method: 'post',
			headers: {
				'Content-Type': 'application/json',
			},
