...
import { ADDRESS_PROVIDER_ID, GIG_IMPL_ID, USER_IMPL_ID } from '../../../helpers/deploy_ids';
import { getACLManager, getAddressProvider, getGigLibraries, getUser, getUserLibraries } from '../../../helpers/contract_getters';
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
	await makeDeployment(func.id, async () => {
		...
		deployments.log(`[Deployments] GIG is registered to ${ gigArtifact.address }`)
		let GigInstance = await getUser(gigArtifact.address);
		deployments.log(`[Deployments] GIG approved MAX_UINT to mUSDC at ${ gigArtifact.address }`)
		waitForTx(
			await GigInstance.approve(mUSDCAddress)
		)
