...
import { ACL_MANAGER_ID, ADDRESS_PROVIDER_ID, USER_IMPL_ID } from '../../../helpers/deploy_ids';
import { getACLManager, getAddressProvider, getUserLibraries } from '../../../helpers/contract_getters';
import { ACL_MANAGER, PROTOCOL_ADMIN_ROLE, USER } from '../../../helpers/constants';
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
	await makeDeployment(func.id, async () => {
		...
		waitForTx(
			await addressProviderInstance.addContract(ACL_MANAGER, aclManagerArtifact.address)
		)
		// give the PROTOCOL_ADMIN_ROLE
		waitForTx(
			await aclManager.grantRole(PROTOCOL_ADMIN_ROLE, aclAdmin)
		)
		deployments.log(`[Deployments] PROTOCOL_ADMIN_ROLE granted to ${ aclAdmin }`)
	})
}
