...
describe('User: becomeBuyer', () => {
    ...
    describe('can become buyer', () => {
        ...
        it('user0 should have buyer role', async () => {
            ...
            expect(isBuyer).to.be.equal(
                true,
                'User is not correctly set to BUYER_ROLE'
            )
        })
        it('user0 should have more buyer time', async () => {
            let userData = await User.getUserByAddress(user0.address)
            expect(userData[2]).to.be.equal(
                timestamp + BUYER_ENTRY_PARAMS.timeAdded,
                'Invalid buyer time added'
            )
        })
        it('user0 should have more buyer invitations', async () => {
            let userData = await User.getUserByAddress(user0.address)
            expect(userData[3]).to.be.equal(
                BUYER_ENTRY_PARAMS.invitationEarned,
        ...
        describe('Entry fees', () => {
            ...
            it('users should have paid entry fees', async () => {
                expect(user0balanceAfter.USDC).to.be.equal(
                    user0BalanceBefore.USDC - BUYER_ENTRY_PARAMS.currencyValue
                )
                expect(user1balanceAfter.USDC).to.be.equal(
                    user1BalanceBefore.USDC - BUYER_ENTRY_PARAMS.currencyValue
                )
                expect(user2balanceAfter.USDC).to.be.equal(
                    user2BalanceBefore.USDC - BUYER_ENTRY_PARAMS.currencyValue
                )
            })
