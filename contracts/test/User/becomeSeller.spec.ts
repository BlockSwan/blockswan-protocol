...
describe('User: becomeSeller', () => {
    ...
    describe('can become buyer', () => {
        beforeEach(async () => {
            ...
            await setup(user0, USER_TEST0, user0BalanceBefore).then(
                async () => {
        ...
        it('user0 should have seller role', async () => {
            ...
            expect(isSeller).to.be.equal(
                true,
                'User is not correctly set to SELLER_ROLE'
            )
        })
        it('user0 should have more seller time', async () => {
            let userData = await User.getUserByAddress(user0.address)
            expect(userData[4]).to.be.equal(
                timestamp + SELLER_ENTRY_PARAMS.timeAdded,
                'Invalid seller time added'
            )
        })
        it('user0 should have more seller invitations', async () => {
            let userData = await User.getUserByAddress(user0.address)
            expect(userData[5]).to.be.equal(
                SELLER_ENTRY_PARAMS.invitationEarned,
                'Invalid invitation earned added'
            )
        })
        describe('Entry fees', () => {
            ...
            it('users should have paid entry fees', async () => {
                expect(user0balanceAfter.USDC).to.be.equal(
                    user0BalanceBefore.USDC - SELLER_ENTRY_PARAMS.currencyValue
                )
                expect(user1balanceAfter.USDC).to.be.equal(
                    user1BalanceBefore.USDC - SELLER_ENTRY_PARAMS.currencyValue
                )
                expect(user2balanceAfter.USDC).to.be.equal(
                    user2BalanceBefore.USDC - SELLER_ENTRY_PARAMS.currencyValue
                )
            })
            it('inviters should have received BSWAN share', async () => {
                ...
                expect(user2balanceAfter.BSWAN).to.be.equal(
                    user2BalanceBefore.BSWAN
                )
            })
            it('inviters should have received BSWAN share', async () => {
                expect(user0balanceAfter.BSWAN).to.be.greaterThan(
