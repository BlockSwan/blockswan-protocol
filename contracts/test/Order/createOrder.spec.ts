...
describe('Order: createOrder', () => {
    ...
    describe('can create an order ', () => {
        ...
        it('should have the correct created timestamp', async () => {
            expect(orderData.invoice.createdAt).to.be.closeTo(
                timestamp,
                20,
                'Invalid timestamp'
            )
        ...
        it('should have the correct price', async () => {
            expect(orderData.invoice.price).to.be.equal(
                gigData.packages[Number(ORDER_TEST0.packageId)].price
            )
        })

        it('should have the correct currency', async () => {
            expect(orderData.invoice.currency).to.be.equal(await dat.currency())
        })
        ...
        describe('Relations', () => {
            ...
            it('orderId should be present at the user level', async () => {
                ...
                expect(userData.buyerOrderIds).to.deep.include(
                    orderData.orderId
                )
            })
        })
        ...
        describe('Balances:', () => {
            ...
            it('buyer should have paid UDSC', async () => {
                expect(buyerBalanceAfter.USDC).to.be.equal(
                    buyerBalanceBefore.USDC -
                        calcOrderPrice(
                            gigData.packages[Number(ORDER_TEST0.packageId)]
                                .price,
                            ORDER_CREATION_PARAMS
                        )
                )
            ...
            it('Order contact should have received the total price', async () => {
                expect(orderBalanceAfter.USDC).to.be.equal(
                    orderBalanceBefore.USDC +
                        calcOrderPrice(
                            gigData.packages[Number(ORDER_TEST0.packageId)]
                                .price,
                            ORDER_CREATION_PARAMS
