...
describe('Order: refund', () => {
	...
	describe("can refund buyer", () => {
		...
		beforeEach(async () => {
			...
			waitForTx(
				await Order.connect(seller.signer).refundOrder(orderData.orderId,orderData.sellerId, orderData.buyerId)
			);
		...
		it("should have refunded the buyer", async () => {
			...
			expect(buyerBalanceAfter.USDC).to.be.equal(buyerBalanceBefore.USDC + calcOrderPrice(Number(orderData.package.price), ORDER_CREATION_PARAMS))
		})
