...
export class Market {
    ...
    async addInvestor() {
        ...
        console.log(
            green().bold(`Investor ${investor.signer?.address} joined!\n`)
        )
        if (LOG_NEW_INVESTOR) {
            investor.print()
        }
    ...
    async getDATinfo(): Promise<Epoch['datInfo']> {
        let obj = {
            reserve:
                (await this.DAT.buybackReserve()).toNumber() / Math.pow(10, 6),
            bswanSupply:
                (await this.DAT.totalSupply()).toNumber() / Math.pow(10, 6),
            buy1BSWAN: this.getBSWANprice(
                Number(ONE_USD) * 100,
                (
                    await this.DAT.estimateBuyValue(Number(ONE_USD) * 100)
                ).toNumber()
            ),
        }
        return {
    ...
    getMarketRisk(index?: number): number {
        ...
        for (let i = 0; i < max; i++) {
            marketRisk += Math.abs(
                (this.epochs[i]?.datInfo?.buy1BSWAN || 0) -
                    (this.epochs[i - 1]?.datInfo?.buy1BSWAN || 0)
            )
        }
        marketRisk /= max + 1
        return marketRisk
    ...
    getMarketReturn(index?: number): number {
        ...
        for (let i = 0; i < max; i++) {
            marketReturn += calculateReturn(
                this.epochs[i - 2]?.datInfo?.buy1BSWAN || 0,
                this.epochs[i - 1]?.datInfo?.buy1BSWAN || 0
            )
        }
        marketReturn /= max + 1
