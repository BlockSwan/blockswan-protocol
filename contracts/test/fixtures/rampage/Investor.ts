...
function getRiskAssessmentAbility(
    age: number
): Investor['riskAssessmentAbility'] {
    if (age < 30) {
...
export class Investor {
    ...
    constructor(id: number, dat: BSWAN, mUSDC: MUSDC) {
        ...
        this.investmentStyle =
            random(0, 1) < 0.5 ? 'aggressive' : 'conservative'
    ...
    async setMaxAllowance() {
        ...
        waitForTx(await this.mUSDC.approve(this.DAT.address, MAX_UINT_AMOUNT))
    }

    async incrUSDCAllowance(currencyValue?: number | string) {
        this.signer.signer.connect(ethers.provider)
        waitForTx(
            await this.mUSDC.approve(
                this.DAT.address,
                currencyValue || MAX_UINT_AMOUNT
            )
        )
    }
    ...
    async mintUSDC(currencyValue?: number | string) {
        this.signer.signer.connect(ethers.provider)
        waitForTx(await this.mUSDC.mint(currencyValue || MIN_INVESTMENT))
    }

    async getUSDCBalance(): Promise<number> {
        let balance = (
            await this.mUSDC.balanceOf(this.signer.address)
        ).toNumber()
        return balance
    }
    ...
    async getUSDCAllowance(): Promise<number> {
        let allowance = (
            await this.mUSDC.allowance(this.signer.address, this.DAT.address)
        ).toNumber()
        return allowance
    }
    ...
    async buyBSWAN(currencyValue: number): Promise<void> {
        ...
        let tokenBought = (
            await this.DAT.estimateBuyValue(currencyValue)
        ).toNumber()
        waitForTx(await this.DAT.buy(this.signer.address, currencyValue, 1))
        console.log(
            blue().bold(
                `${this.signer.address} exchanged ${
                    currencyValue / Math.pow(10, 6)
                } mUSDC for ${tokenBought / Math.pow(10, 6)} BSWAN\n`
            )
        )
        console.log(
            bold(
                `\t1 BSWAN = ${
                    currencyValue / tokenBought / Math.pow(10, 6)
                } mUSDC\n`
            )
        )
    }
