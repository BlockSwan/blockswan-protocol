...
describe('DAT: pay', () => {
    ...
    beforeEach(async () => {
        ...
        waitForTx(
            await testEnv.mUSDC.connect(investor.signer).mint(payAmount * 10)
        )
        waitForTx(
            await testEnv.mUSDC.connect(buyer.signer).mint(payAmount * 10)
        )
        waitForTx(
            await testEnv.mUSDC
        ...
        waitForTx(
            await testEnv.mUSDC
                .connect(investor.signer)
                .approve(testEnv.dat.address, MAX_UINT_AMOUNT)
        )
        waitForTx(
            await testEnv.dat
                .connect(buyer.signer)
                .buy(buyer.address, payAmount * 2, 1)
        )
    })
    ...
    describe('on pay', () => {
        ...
        beforeEach(async () => {
            investorBalanceBefore = (
                await testEnv.dat.balanceOf(investor.address)
            ).toNumber()

            waitForTx(
                await testEnv.dat
                    .connect(investor.signer)
                    .pay(ZERO_ADDRESS, payAmount)
            )
        })
    ...
    it('can make a tiny payment', async () => {
        waitForTx(
            await testEnv.dat.connect(investor.signer).pay(investor.address, 1)
        )
    })
})
