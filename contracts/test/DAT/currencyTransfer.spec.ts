...
describe('DAT: currencyTransfer', () => {
	...
	beforeEach(async () => {
		...
		waitForTx(await testEnv.mUSDC.connect(user.signer).mint(initialTokenBalance))
		waitForTx(await testEnv.mUSDC.connect(user.signer).approve(testEnv.dat.address, MAX_UINT_AMOUNT));
	});
	...
	it("DAT has no currency yet", async () => {
		const balance = (await testEnv.mUSDC.balanceOf(testEnv.dat.address)).toNumber();
		expect(balance).to.be.equal(0);
	});
	...
	describe("can transfer currency into the contract", () => {
		...
		beforeEach(async () => {
			waitForTx(await testEnv.mUSDC.connect(user.signer).transfer(testEnv.dat.address, currencyToSend))
		});
		...
		it("User spent currency", async () => {
			const balance = (await testEnv.mUSDC.balanceOf(user.address)).toNumber()
