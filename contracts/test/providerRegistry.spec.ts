...
describe("provider_registry", function () {
	...
	it('Tries to register an addresses provider with id 0 (revert expected)', async () => {
		...
		await expect(
			Registry.connect(registryOwner).registerAddressProvider(NEW_ADDRESSES_PROVIDER_ADDRESS, '0')
	...
	it('Registers a mock addresses provider', async () => {
		...
		expect(await Registry.getAddressProviderIdByAddress(NEW_ADDRESSES_PROVIDER_ADDRESS)).to.be.eq(
			NEW_ADDRESSES_PROVIDER_ID_2
		);
		...
		expect(await Registry.getAddressProviderById(NEW_ADDRESSES_PROVIDER_ID_2)).to.be.equal(
			NEW_ADDRESSES_PROVIDER_ADDRESS,
	...
	it('Registers users[2] as another addresses provider', async () => {
		...
		expect(await Registry.connect(registryOwner).registerAddressProvider(users[2].address, NEW_ADDRESSES_PROVIDER_ID_3))
			.to.emit(Registry, 'ev_addresses_provider_registered')
	...
	it('Removes the mock addresses provider', async () => {
		...
		expect(
			await Registry.getAddressProviderIdByAddress(NEW_ADDRESSES_PROVIDER_ADDRESS)
		).to.be.equal(NEW_ADDRESSES_PROVIDER_ID_2);

		expect(await Registry.connect(registryOwner).unregisterAddressProvider(NEW_ADDRESSES_PROVIDER_ADDRESS))
	...
	it('Tries to remove an already unregistered AddressProvider (revert expected)', async () => {
		...
		await expect(
			Registry.connect(registryOwner).unregisterAddressProvider(NEW_ADDRESSES_PROVIDER_ADDRESS)
	...
	it('Tries to add an already registered AddressProvider with a different id (revert expected)', async () => {
		...
		const id = await Registry.getAddressProviderIdByAddress(AddressProvider.address);
		expect(id).not.to.be.eq(0);

		const providers_before = await Registry.getAddressProvidersList();

		await expect(
			Registry.connect(registryOwner).registerAddressProvider(AddressProvider.address, NEW_ADDRESSES_PROVIDER_ID_2)
		).to.be.revertedWith(ADDRESS_PROVIDER_ALREADY_ADDED);

		const providers_after = await Registry.getAddressProvidersList();

		expect(await Registry.getAddressProviderIdByAddress(AddressProvider.address)).to.be.eq(id);
	...
	it('Tries to add an AddressProvider with an already used id (revert expected)', async () => {
		...
		await expect(Registry.connect(registryOwner).registerAddressProvider(users[5].address, id)).to.be.revertedWith(
			INVALID_ADDRESS_PROVIDER_ID
		);
		...
		for (let i = 0; i < providers.length; i++) {
			const id = (await Registry.getAddressProviderIdByAddress(providers[i])).toNumber();
			if (id > 0) {
	...
	it('Removes the last addresses provider', async () => {
		...
		const provider_to_remove_id = await Registry.getAddressProviderIdByAddress(provider_to_remove);

		expect(await Registry.connect(registryOwner).unregisterAddressProvider(provider_to_remove))
			.to.emit(Registry, 'ev_addresses_provider_unregistered')
