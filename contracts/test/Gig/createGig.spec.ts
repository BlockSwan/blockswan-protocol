...
import { BUYER_ENTRY_PARAMS, BUYER_ROLE, DEFAULT_BALANCE, GIG_CREATION_PARAMS, GIG_TEST0, GIG_TEST1, GIG_TEST2, USER_BIGINVITER_TEST, USER_TEST0, USER_TEST1, USER_TEST2, XP_VALUES } from '../../helpers/constants';
import { maxApproveUser } from '../../helpers/init_helpers';
import { getBalances, mintAndApproveDAT, setupSeller, setupUser } from '../../helpers/test_helpers';
import { Balance, GigInput, ProtocolErrors, SignerWithAddress, TestEnv, UserInput } from '../../helpers/types';
...
describe('Gig: createGig', () => {
	...
	let { User, XP, ProtocolConfigurator, mUSDC, dat, ACLManager, Gig } = testEnv;
	...
	describe("can create a gig", () => {
		...
		beforeEach(async () => {
			...
			async function setup(user: SignerWithAddress, userArgs: UserInput, gigArgs: GigInput, userBalance: Balance) {
		...
		it("should have the correct packages", async () => {
			let gigData = await Gig.getGigById(0);
			const expected = gigData[7].map((elem: any) => { return { price: elem.price, timeDelivery: elem.timeDelivery } });
			const actual = GIG_TEST0[1].map((elem: any) => { return { price: elem.price, timeDelivery: elem.timeDelivery } });
		...
		it("user0 should have more xp", async () => {
			...
			expect(userXP.toNumber()).to.be.equal(XP_VALUES[2].amount + XP_VALUES[1].amount);
		})
		...
		it("should have added the gig to the user", async () => {
			...
			expect(userData[8][0].toNumber()).to.be.equal(0, "gig id is missing at the user level");
			expect(userData[8][1].toNumber()).to.be.equal(3, "gig id is missing at the user level");
		})
		...
		describe("Creation fees", () => {
			...
			it("users should have paid creation fees", async () => {
				...
				expect(user0balanceAfter.USDC).to.be.equal(user0BalanceBefore.USDC - Number(GIG_CREATION_PARAMS.currencyValue))
				expect(user1balanceAfter.USDC).to.be.equal(user1BalanceBefore.USDC - Number(GIG_CREATION_PARAMS.currencyValue))
			})
